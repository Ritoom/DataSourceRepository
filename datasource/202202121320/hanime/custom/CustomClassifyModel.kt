...
class CustomClassifyModel : IClassifyModel {
                    ...
                    for (anime in animeRealItem) {
                        when(anime.tagName()){
                            "a" -> {
                                ...
                                image.actionUrl = anime.select("img")[1].attr("src")
                            }
                            "div" -> {
                ...
                if (r.attr("class")
                    ...
                    if(r.children().size > 0){
                        ...
                        if (page.toIntOrNull() != null && page.toIntOrNull()!! > 0){
                            val pageUrl = const.actionUrl.ANIME_SEARCH()+"/" + r.child(0)
    ...
    override suspend fun getClassifyTabData(): ArrayList<ClassifyBean> {
        ...
        val url = "${const.MAIN_URL()}${const.actionUrl.ANIME_SEARCH()}"
        val document = JsoupUtil.getDocument(url)
                                        ...
                                        "label" -> {
                                            if (classifyTabList.size > 0){
                                                classifyTabList[classifyTabList.size-1].classifyDataList.add(
                                                    ClassifyTab1Bean(
                                                        "${const.actionUrl.ANIME_SEARCH()}?tags%5B%5D=${tag.text()}",
                                                        "",
                                                        tag.text())
                                                )
                                            }
                                    ...
                                    when (tag.tagName()){
                                        ...
                                        "label" -> {
                                            if (classifyTabList.size > 0){
                                                classifyTabList[classifyTabList.size-1].classifyDataList.add(
                                                    ClassifyTab1Bean(
                                                        "${const.actionUrl.ANIME_SEARCH()}?brands%5B%5D=${tag.text()}",
                                                        "",
                                                        tag.text())
                                                )
                                            }
        ...
        val tags = document.getElementById("tags")

        val brands = document.getElementById("brands")
        if (brands != null){
