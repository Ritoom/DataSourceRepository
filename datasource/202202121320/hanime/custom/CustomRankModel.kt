...
class CustomRankModel : IRankModel {
    ...
    override suspend fun getRankTabData(): ArrayList<TabBean> {
        ...
        tabList.add(
            TabBean(
                const.actionUrl.ANIME_RANK()+"?genre=全部&sort=本日排行",
                "",
                "本日排行榜"
            )
        )
        tabList.add(
            TabBean(
                const.actionUrl.ANIME_RANK()+"?genre=全部&sort=本週排行",
                "",
                "本週排行榜"
            )
        )
        tabList.add(
            TabBean(
                const.actionUrl.ANIME_RANK()+"?genre=全部&sort=本月排行",
                "",
                "本月排行榜"
            )
        )
        tabList.add(
            TabBean(
                const.actionUrl.ANIME_RANK()+"?genre=全部&sort=觀看次數",
                "",
                "觀看次數排行榜"
            )
        )
