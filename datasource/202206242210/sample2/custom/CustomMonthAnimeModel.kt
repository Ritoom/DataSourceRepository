        ...
        for (i in area.indices) {
            ...
            for (j in elements.indices) {
                when (elements[j].className()) {
                    "img", "imgs" -> {
                        monthAnimeList.addAll(CustomParseHtmlUtil.parseImg(elements[j], url))
                    }
                    "fire l" -> {       //右侧前半tab内容
                    ...
                    "dnews" -> {       //右侧后半tab内容，cover4
                        monthAnimeList.addAll(CustomParseHtmlUtil.parseDnews(elements[j], url))
                    }
                    "topli" -> {       //右侧后半tab内容，cover5
                        monthAnimeList.addAll(CustomParseHtmlUtil.parseTopli(elements[j]))
                    }
                    "pages" -> {
                        pageNumberBean =
                            CustomParseHtmlUtil.parseNextPages(elements[j])
                    }
                }
            }
        }
